...
from rest_framework.decorators import api_view, permission_classes
from rest_framework.permissions import AllowAny
from .models import WaterQualityData, Reading
from .serializers import WaterQualityDataSerializer, ReadingSerializer
import pytz
import os
...
# File to store calibration state
CALIBRATION_STATE_FILE = 'calibration_state.json'

# Global variable to track calibration thread
...
def load_calibration_state():
    ...
    try:
        if os.path.exists(CALIBRATION_STATE_FILE):
            with open(CALIBRATION_STATE_FILE, 'r') as f:
                return json.load(f)
        return {'calibration_active': False, 'calibration_data': None}
    except Exception as e:
        logger.error(f"Error loading calibration state: {str(e)}")
        return {'calibration_active': False, 'calibration_data': None}
...
def save_calibration_state(state):
    ...
    try:
        ...
        logger.error(f"Error saving calibration state: {str(e)}")
...
def water_analyzer_dashboard(request):
    """Water analyzer dashboard hardcoded for user ID 25"""
    try:
        # Fixed user ID to 25
        user_id = '25'
        
        # Fetch external API data for user 25
        url = f'https://impulseengineers.com/app/get_cod_bod_tss_spm_data/{user_id}/'
        ...
        if response.status_code == 200:
            ...
            context = {
                'user_id': user_id,
                'ph': data.get('ph_value', '00.00'),  # API returns ph_value
                'cod': data.get('cod', '00.00'),
                'bod': data.get('bod', '00.00'),
                'tss': data.get('tss', '00.00'),
                'flow': data.get('flow_rate', '00.00'),
                'flow_total': data.get('flow_total', '00.00'),
                'daily_flow': data.get('daily_flow', data.get('flow_total', '00.00')),
                'last_updated': data.get('created_date', '--'),
                'calibration_active': False
            }

            # Handle calibration mode submission
            if request.method == 'POST' and 'submit_calibration' in request.POST:
                calibrated_data = {
                    'user_id': 25,  # Ensure it's an integer
                    'ph': float(request.POST.get('ph', '0.00')),  # Send as 'ph', not 'ph_value'
                    'cod': float(request.POST.get('cod', '0.00')),
                    'bod': float(request.POST.get('bod', '0.00')),
                    'tss': float(request.POST.get('tss', '0.00')),
                    'flow_rate': float(request.POST.get('flow', '0.00')),
                    'flow_total': float(request.POST.get('flow_total', '0.00')),
                    'daily_flow': float(request.POST.get('daily_flow', request.POST.get('flow_total', '0.00'))),
                    'spm': 0,  # Add spm field as shown in your sample
                    'record': True  # Add record field as shown in your sample
                }
                
                # Submit calibrated data to external API
                try:
                    submit_response = requests.post(
                        'https://impulseengineers.com/app/record_new_cod_bod_tss_data/',
                        json=calibrated_data,  # Use json parameter instead of data
                        headers={'Content-Type': 'application/json'},
                        timeout=10
                    )
                    if submit_response.status_code == 200:
                        context.update(calibrated_data)
                        context['calibration_active'] = True
                        messages.success(request, f'Calibration data submitted successfully for User {user_id}!')
                    else:
                        messages.error(request, f'Failed to submit calibration data: {submit_response.status_code}')
                        logger.error(f"API response: {submit_response.text}")
                except requests.exceptions.RequestException as e:
                    logger.error(f"Error submitting calibration data: {e}")
                    messages.error(request, 'Error submitting calibration data')

        else:
            # If API fails, use fallback values for user 25
            context = {
                'user_id': user_id,
                'ph': '00.00',
                'cod': '00.00',
            ...
            logger.warning(f"Failed to fetch data from external API for user {user_id}")

    except Exception as e:
        logger.error(f"Error in water_analyzer_dashboard for user 25: {e}")
        context = {
            'user_id': '25',
            'ph': '00.00',
            'cod': '00.00',
...
# Updated get_external_data function in views.py
@api_view(['GET'])
@permission_classes([AllowAny])
def get_external_data(request):
    """Proxy external API hardcoded to user ID 25"""
    try:
        # Fixed user ID to 25
        user_id = '25'
        
        # Fetch from external API with user ID 25
        url = f'https://impulseengineers.com/app/get_cod_bod_tss_spm_data/{user_id}/'
        response = requests.get(url, timeout=10)
        response.raise_for_status()
        data = response.json()
        
        # Add user_id to response and ensure daily_flow is included
        data['user_id'] = user_id
        
        # If daily_flow is not in the response, use flow_total as fallback
        if 'daily_flow' not in data:
            data['daily_flow'] = data.get('flow_total', '00.00')
        
        return JsonResponse(data)
    except requests.exceptions.RequestException as e:
        logger.error(f"Error fetching external data for user 25: {str(e)}")
        return JsonResponse({"error": "Failed to fetch external data"}, status=500)
    except Exception as e:
        logger.error(f"Unexpected error in get_external_data: {str(e)}")
...
def calibration_status(request):
    ...
    return JsonResponse({
        ...
        'calibration_data': state['calibration_data']
    })

def send_calibrated_data(calibration_data):
    """
    Background task to send calibration data every 5 seconds.
    """
    submit_url = "https://www.impulseengineers.com/app/record_new_cod_bod_tss_data/"
    state = load_calibration_state()
    while state['calibration_active']:
        try:
            headers = {'Content-Type': 'application/json'}
            response = requests.post(submit_url, json=calibration_data, headers=headers, timeout=5)
            if response.status_code == 200:
                logger.info(f"Successfully sent calibrated data: {calibration_data}")
            else:
                logger.error(f"Server response error for calibrated data: {response.status_code}, Response: {response.text}")
        except Exception as e:
            logger.error(f"Error sending calibrated data: {str(e)}")
        time.sleep(5)
        state = load_calibration_state()
...
def reset_calibration(request):
    """
    Reset calibration by clearing state and stopping the background thread.
    No authentication required.
    """
    ...
    if request.method == 'POST':
        try:
            ...
            if calibration_thread:
                calibration_thread.join(timeout=1)
                calibration_thread = None
                logger.info("Calibration reset")
            return JsonResponse({'status': 'success', 'message': 'Calibration reset successful'})
        except Exception as e:
            logger.error(f"Error resetting calibration: {str(e)}")
            return JsonResponse({'status': 'error', 'message': str(e)}, status=500)
    return JsonResponse({'status': 'error', 'message': 'Invalid request method'}, status=405)

# Updated submit_data function to handle user ID 25 and daily_flow
@api_view(['POST'])
@permission_classes([AllowAny])
def submit_data(request):
    """
    Handle calibration data submission hardcoded for user ID 25.
    """
    global calibration_thread
    if request.method == 'POST':
        try:
            data = request.data
            user_id = '25'  # Fixed to user ID 25
            ...
            bod = parse_float(data.get('bod'))
            tss = parse_float(data.get('tss'))
            flow_rate = parse_float(data.get('flow_rate'))
            ...
            if flow_rate is not None and flow_rate < 0:
                return JsonResponse({'success': False, 'error': 'Flow rate cannot be negative'}, status=400)
            if daily_flow is not None and daily_flow < 0:
                return JsonResponse({'success': False, 'error': 'Daily Flow cannot be negative'}, status=400)
            if flow_total is not None and flow_total < 0:
                return JsonResponse({'success': False, 'error': 'Flow total cannot be negative'}, status=400)
            ...
            state = load_calibration_state()
            state['calibration_data'] = calibration_data
            state['calibration_active'] = True
            save_calibration_state(state)

            # Start background thread for sending calibration data
            if not calibration_thread or not calibration_thread.is_alive():
                calibration_thread = threading.Thread(
                    target=send_calibrated_data, 
                    args=(calibration_data,), 
                    daemon=True
                )
                calibration_thread.start()
                logger.info(f"Started calibration thread for user_id 25")
...
def send_calibrated_data(calibration_data):
    """
    Background task to send calibration data every 5 seconds to external API for user 25.
    """
    user_id = 25  # Fixed to user ID 25
    submit_url = "https://www.impulseengineers.com/app/record_new_cod_bod_tss_data/"
    ...
    while state['calibration_active']:
        try:
            ...
            # Add spm field if not present (as shown in your sample)
            if 'spm' not in api_data:
                api_data['spm'] = 0
            ...
            if response.status_code == 200:
                logger.info(f"Successfully sent calibrated data for user_id 25: {api_data}")
            else:
                logger.error(f"Server response error for user_id 25: {response.status_code}, Response: {response.text}")
                
        except Exception as e:
            logger.error(f"Error sending calibrated data for user_id 25: {str(e)}")
            
        # Wait 5 seconds before next transmission
        time.sleep(5)
        
        # Reload state to check if calibration is still active
        state = load_calibration_state()
    
    logger.info(f"Stopped sending calibrated data for user_id 25")


# Add this new view for getting user list (optional)
@api_view(['GET'])
@permission_classes([AllowAny])
def get_users_list(request):
    """
    Return list of available users for dropdown.
    This is optional - you can hardcode users in frontend if preferred.
    """
    try:
        if request.user.is_authenticated and request.user.is_admin:
            # If admin, show all users
            users = CustomUser.objects.filter(role=CustomUser.USER).values('id', 'username', 'first_name', 'last_name')
        else:
            # For non-admin users, just return a basic list or current user
            users = [{'id': i, 'username': f'User {i}', 'first_name': '', 'last_name': ''} for i in range(1, 11)]
...
class WaterQualityDataViewSet(viewsets.ModelViewSet):
    ...
    def latest(self, request):
        ...
        try:
            ...
            logger.error(f"Error in water quality latest action for user_id {user_id}: {str(e)}")
            return Response({"error": str(e)}, status=500)
...
class ReadingViewSet(viewsets.ModelViewSet):
    ...
    def latest(self, request):
        ...
        try:
            ...
            logger.error(f"Error in readings latest action for user_id {user_id}: {str(e)}")
            return Response({"error": str(e)}, status=500)
